...
1. 根据行业获取金融领域上市公司股票代码列表和财务报表（按报告期）
 - 银行
 - 券商
 - 保险
2. 获取所有金融领域上市公司股票财务指标（按季度或年度）
 - 银行
 - 券商
 - 保险
3. 获取所有非金融领域上市公司股票财务报表（按报告期）
   **当前提示：获取财务数据失败: 所获取数据的权限类型未知**
 - 资产负债表
 - 利润表
 - 现金流量表
4. 获取所有非金融领域上市公司股票财务指标（按季度或年度）
5. 实际使用时，需要替换代码中的测试代码
 - 目前获取的是列表前10个公司的财务数据