...
def get_stock_industry(date):
    ...
    industry_dict = jq.get_industry(security=stock_list, date=date)
    print("原始数据:")
    print(industry_dict)
    print()
    ...
    # 将字典转换为DataFrame
    data_rows = []
    for stock_code, industries in industry_dict.items():
        for source, industry_info in industries.items():
            row = {
    ...
    # 创建保存目录
    save_dir = get_path('stock_industry')
    ...
    print(f"股票所属行业列表已保存至: {save_path}")
    print()  # 空行分隔

if __name__ == "__main__":
    date = '2025-08-27'
    get_stock_industry(date)